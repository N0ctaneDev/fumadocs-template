...
// fumadocs-mdx reads this at bundler time via esbuild.
// Must be static named exports — cannot be a runtime loop.
// This script generates exactly those static exports from __CONFIG__.ts.
...
// @ = /src so @/lib/... resolves to src/lib/...
// __CONFIG__ resolves from root via tsconfig paths
// No i18n in loader — English only for now, wire back when needed
// No getSection re-exports — layout.tsx uses useMode() with params.project directly
...
for (const project of PROJECTS) {
  sourcesLines.push(
    `  // eslint-disable-next-line @typescript-eslint/no-explicit-any`,
    `  "${project.slug}": ${toCamel(project.slug)}Source as any,`,
  );
}
...
sourcesLines.push(
  `// ─── Unified LLM text helper ─────────────────────────────────────────────────`,
  `// Used by app/llms-full.txt/route.ts — auto-updated when projects change`,
  `// eslint-disable-next-line @typescript-eslint/no-explicit-any`,
  `const LLM_GETTERS: Record<string, (page: any) => Promise<string>> = {`,
);

for (const project of PROJECTS) {
  const Pascal = toPascal(project.slug);
  sourcesLines.push(`  "${project.slug}": get${Pascal}LLMText,`);
}

sourcesLines.push(
  `};`,
  ``,
  `/** Gets plain LLM text for every page across all projects. */`,
  `export async function getAllLLMText(): Promise<string[]> {`,
  `  const sections: string[] = [];`,
  `  for (const [slug, source] of Object.entries(SOURCE_REGISTRY)) {`,
  `    const getter = LLM_GETTERS[slug];`,
  `    if (!getter) continue;`,
  `    // eslint-disable-next-line @typescript-eslint/no-explicit-any`,
  `    const texts = await Promise.all((source as any).getPages().map(getter));`,
  `    sections.push(...texts);`,
  `  }`,
  `  return sections;`,
  `}`,
