  ...
  return (tree: any, file: any) => {
    ...
    visit(tree, "mdxJsxFlowElement", (node: any) => {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        imports.push({
          varName: sourceCodeVarName,
          importPath: sourceCodePath,
        });
        node.attributes.push(
          generateExpressionAttribute("code", sourceCodeVarName),
        );
      } else {
        const sourceCode = fs.readFileSync(absolutePath, "utf-8");
        node.attributes.push(
          generateExpressionAttribute("code", JSON.stringify(sourceCode)),
        );
      }
    });
